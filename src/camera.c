...
void    configure_camera(t_env *env)
{
    ...
    */
    env->camera.origin = subtract_vector(add_vector(create_vector(0, 0, 0), add_vector(multiply_vector_by_scalar(env->camera.direction, 1.0 / env->camera.half_view), multiply_vector_by_scalar(create_vector(0, 1, 0), env->camera.height / 2.0))), multiply_vector_by_scalar(create_vector(1, 0, 0), env->camera.width / 2.0));
    env->camera.xi = env->camera.width / (double)env->window.width;
    env->camera.yi = env->camera.height / (double)env->window.height;
}